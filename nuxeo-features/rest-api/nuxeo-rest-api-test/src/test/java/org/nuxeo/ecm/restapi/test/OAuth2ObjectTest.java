...
@LocalDeploy({"org.nuxeo.ecm.platform.restapi.test:test-oauth2provider-config.xml",
              "org.nuxeo.ecm.platform.restapi.test:test-directory-contrib.xml"})
...
public class OAuth2ObjectTest extends BaseTest {
    ...
    protected static final String TOKEN_SERVER_URL = "https://test.oauth2.provider/token";
    ...
    public void iCanGetAuthData() throws IOException {
        ...
        assertEquals(TOKEN_SERVER_URL, node.get("tokenServerURL").getTextValue());
        assertEquals(AUTHORIZATION_SERVER_URL, node.get("authorizationServerURL").getTextValue());
        assertTrue(node.get("scopes").size() == 2);
        assertEquals(getScopeUrl(0), node.get("scopes").get(0).getTextValue());
        assertEquals(getScopeUrl(1), node.get("scopes").get(1).getTextValue());
